...
df_products = pd.read_csv('../resources/clean_data/clean_products.csv', encoding='utf-8')
...
st.markdown(f"**Categoría:** `{best_product_named['product_category_name']}`")
...
st.markdown(f"**Categoría:** `{worst_product_named['product_category_name']}`")
...
comparison_df = pd.DataFrame([
    {
        ...
        'Categoría': best_product_named['product_category_name'],
        'Puntuación promedio': best_product_named['avg_score']
    },
    {
        'Tipo': 'Peor producto',
        'product_id': worst_product_named['product_id'],
        'Categoría': worst_product_named['product_category_name'],
